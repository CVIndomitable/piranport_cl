# 皮兰港 版本路线图

本文档按版本顺序列出每个版本的核心内容。已完成的详细 Phase 记录见 `phases_archive.md`，代码审查记录见 `code_review_*.md`。

---

## 分支策略（自 v1.0 起）

- **`main` = 稳定主线**：当前版本 **v1.0.0**。只接受 bug 修复、数值微调、文档更新，不加新玩法。目标是跑通全流程零崩溃。
- **`dev` = 测试线**：版本号 `1.1.0-dev`，在 main 之上试验新玩法/新系统。稳定后合并回 main 成为 v1.1.0。
- 后续新大版本遵循同模式：main 发 `N.0`，dev 孵 `N+0.1`。

---

## 已完成版本

| 版本 | 代号 | 状态 | 核心内容 |
|------|------|------|---------|
| v1.0.0 | Stable | DONE | 基于 v0.0.12 的稳定化收官：v0.0.1–v0.0.12 全部 alpha 内容合并为首个稳定版，进入 bug 修复阶段 |
| v0.0.1-alpha | MVP | DONE | 基础注册、世界生成、合成、舰装核心GUI、火炮战斗 |
| v0.0.2-alpha | Torpedo | DONE | 鱼雷系统、舰装栏GUI完善、负重平衡、装填机制 |
| v0.0.3-alpha | Kitchen | DONE | 食物烹饪 + Buff系统、作物种植、加工站 |
| v0.0.4-alpha | Aviation | DONE | 航空系统（4种飞机+编组GUI+火控+AI）+ Patchouli手册 |
| v0.0.5-alpha | Combat+ | DONE | 装填进度Decorator、装填加速/高速规避Buff、Buff食物注册 |
| v0.0.6-alpha | Skin | DONE | 皮肤/模型渲染系统（SkinManager + SkinOverlayLayer + SkinCoreItem） |
| v0.0.7-alpha | Aviation+ | DONE | 侦察机视角切换、空战、编队跟随侦察机、加工站自动化 |
| v0.0.8-alpha | Salt & Rice | DONE | 稻田种植、盐蒸发系统、条件性盐矿生成、全面代码审查修复(35项P0-P3) |
| v0.0.9-alpha | Dungeon | DONE | 副本系统（大厅/实例/节点战斗/传送门/箱子船/钥匙/回城卷轴）+ JEI兼容 + 武器冷却条 + 友军伤害防护 + 彩色描边 + 击落归因 |
| v0.0.10-alpha | Arsenal | DONE | 导弹系统 + 深弹系统 + 命名鱼雷 + 武器/弹药合成台 + 装填设施 + 特殊道具(烟幕/照明弹/电磁炮/损管/维修台/足球套装等) + 皮肤系统 + 全局代码审查修复(47项) |
| v0.0.11-alpha | Ruins | DONE | 主世界四类遗迹(传送门/补给站/前哨站/深海基地) + 9种深海NPC + 集群AI + 抛物线/追踪弹道 + 战利品系统 + 占位物品(国旗/碎片/档案) + 调试命令 + 舰娘NPC框架 |
| v0.0.12-alpha | Gastronomy | DONE | 按 7.1 食物表对齐：新创造栏"舰娘食物"；新增 5 种作物（扁豆/红豆/芹菜/黑麦 + 桃树+桃子自然生成） + ~20 种食材/调料/中间品 + ~10 道菜品；厨锅/砧板/石磨/工作台/烟熏/酿造 60+ 合成 |

---

## 参考

- 已完成 Phase 详情：`phases_archive.md`
- 代码审查归档：
  - `code_review_2026-03-31.md`
  - `code_review_2026-04-04.md`
  - `code_review_2026-04-05.md`
  - `code_review_2026-04-10.md`
  - `code_review_2026-04-17.md`
  - `code_review_missile_2026-04-18.md`
  - `code_review_kitchen_2026-04-18.md`
  - `code_review_workbench_2026-04-18.md`
  - `code_review_dungeon_2026-04-18.md`
