...
public class TorpedoEntity extends ThrowableItemProjectile {
    ...
    // isCurrentlyGlowing removed for 1.20.1
        ...
        if (!level().isClientSide() && !exploded) {
            ...
            if (magnetic) {
                ...
                if (target instanceof LivingEntity living) {
                    living.addEffect(new MobEffectInstance(ModMobEffects.FLOODING.get(), 60, 0));
                }
                discard();
            }
            notifyOwner(target);
    ...
    public void addAdditionalSaveData(CompoundTag tag) {
        ...
        if (sourceAircraftName != null) {
            tag.putString("SourceAircraftName",
                    Component.Serializer.toJson(sourceAircraftName));
        }
    }
        ...
        if (tag.contains("SourceAircraftName")) {
            try {
                sourceAircraftName = Component.Serializer.fromJson(
                        tag.getString("SourceAircraftName"));
            } catch (Exception e) {
