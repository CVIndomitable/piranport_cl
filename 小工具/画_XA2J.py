...
def draw_xa2j(loaded: bool):
    ...
    # ---------- 2. 外侧长翼（机腹下缘另起一条，后掠收窄）----------
    # 翼根从 y=23 起，比机腹最低一行（y=21）低两格：中间那格留给描边当接缝，
    # 这样机翼是"挂在机腹下面"的一条独立翼面，而不是机身自己往下鼓出来的一块。
    # 翼面比机身暗一档，翼尖再收窄，后掠角约 25 度。
    for i, x in enumerate(range(5, 24)):
        y = 23 + i * 5 // 19
        th = 3 - i * 2 // 19
        rect(im, x, y, x, y + th, DARK)
        px(im, x, y, MID)                       # 翼前缘受光
        px(im, x, y + 1, MID_D)                 # 翼面中段
        px(im, x, y + th, DEEP)                 # 翼后缘阴影
    # 翼尖收口
    rect(im, 23, 27, 24, 28, DEEP)

    # ---------- 3. 发动机短舱（机翼前缘挂载）----------
    # 外侧翼短舱（画面右下），整流罩呈胶囊形；和机翼一样下移，贴住新翼前缘
    rect(im, 18, 26, 21, 29, METAL)
    rect(im, 18, 26, 21, 26, METAL_L)           # 罩体上缘受光
    rect(im, 18, 27, 21, 27, LIGHT)             # 受光面到主体的过渡
    rect(im, 18, 29, 21, 29, METAL_D)           # 罩体下缘暗部
    px(im, 22, 27, METAL_D)
    px(im, 22, 28, METAL_D)
    # 内侧短舱（贴近机腹，挂在机翼根部上侧）
    rect(im, 9, 23, 13, 26, METAL)
    rect(im, 9, 23, 13, 23, METAL_L)
    rect(im, 9, 25, 13, 25, MID_D)
    rect(im, 9, 26, 13, 26, METAL_D)
    ...
    # ---------- 7. 弹舱挂弹（装填态独占）----------
    if loaded:
        # 内翼短舱后侧挂一枚炸弹，向下鼓出：和空膛态形成明显的外形差，
        # 而不只是靠色差——玩家扫一眼背包就能看出有没有带弹。
        rect(im, 16, 24, 20, 25, METAL)
        rect(im, 16, 24, 20, 24, METAL_L)       # 弹体上缘受光
        rect(im, 16, 25, 20, 25, METAL_D)       # 弹体下缘暗部
        px(im, 15, 24, METAL_D)                 # 弹头/弹尾暗色收口
        px(im, 15, 25, METAL_D)
        px(im, 21, 24, METAL_D)
        px(im, 21, 25, METAL_D)
