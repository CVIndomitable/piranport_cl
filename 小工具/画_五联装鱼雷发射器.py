#!/usr/bin/env python3
"""重画「五联装鱼雷发射器」（quintuple_torpedo_launcher）物品贴图 —— 从原画直接降采样。

原画: https://www.zjsnrwiki.com/wiki/五联轻型鱼雷
      Equip_L_553.png（512×512，缓存于 ~/IndomitableCache/zjsnrwiki原画/）
      内容 bbox x76-431 y173-371 = 356×199 —— 银灰色五联轻型鱼雷发射管，
      五根圆管朝左下、中部灰色固定带、右上卡扣阵列，2:1 扁构图。

管线与 画_法国四联380毫米炮.py 一致:
  裁内容 bbox → 预乘 alpha 面积平均降采样（满宽 32 → 32×18）→
  量化 48 色 → 画布贴底摆放（同 french_quad 扁构图排版）→
  最外圈 8 邻域补纯黑描边。

两态: 本体逐像素一致；装填态无底部图标，空膛态在 (12,24) 盖项目标准
  BURST 空膛叹号（9×8，与 chinese_twin / german_twin / french_quad 标准件
  逐像素一致），盖在描边之后（先 finish 再盖，叹号叠在本体底部上）。

产出（模型引用关系）:
  quintuple_torpedo_launcher.json        layer0 → _empty（默认空膛，带叹号）
  quintuple_torpedo_launcher_loaded.json layer0 → _loaded（装填，无叹号）
  quintuple_torpedo_launcher.png         基础态 = 本体无叹号（与炮类约定一致）
"""
...
SRC = '/Users/lianran/IndomitableCache/zjsnrwiki原画/Equip_L_553.png'
SRC_URL = 'https://0v0.zjsnrwiki.com/images/3/33/Equip_L_553.png'  # 仅缓存缺失时用一次
OUT_MOD = '/Users/lianran/apps/皮兰港实验/测试版/src/main/resources/assets/piranport/textures/item/'
OUT_ART = '/Users/lianran/apps/皮兰港实验/美术素材/Item（图标）/鱼雷发射管/'
...
def quantize(arr):
    """量化颜色到 N_COLORS 档，贴齐范例贴图 30~110 色的颗粒质感；alpha 保留软边。"""
    h, w = arr.shape[:2]
...
def add_outline(arr):
    ...
    用 padding 而不是 np.roll：本体贴底摆放（ART_BOTTOM=31），roll 会把
    底缘描边绕回第 0 行，在图标顶部留下一条孤立黑条。
    """
    solid = arr[:, :, 3] > 8
    ...
    for dy in (0, 1, 2):
        for dx in (0, 1, 2):
            grow |= padded[dy:dy + solid.shape[0], dx:dx + solid.shape[1]]
    edge = grow & ~solid
    arr[edge] = (0, 0, 0, 255)
    return arr
...
if __name__ == '__main__':
    # base / loaded 本体无叹号，empty 盖标准空膛叹号；三份本体逐像素一致
    for name, with_burst in [
        ('quintuple_torpedo_launcher.png', False),
        ('quintuple_torpedo_launcher_loaded.png', False),
        ('quintuple_torpedo_launcher_empty.png', True),
    ]:
        im = build(with_burst)
